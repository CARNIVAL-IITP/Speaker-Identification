...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            cls.resume_valid(
                checkpoint=output_dir / "valid.acc.best.pth", #checkpoint.pth valid.acc.best.pth
                model=model,
                optimizers=optimizers,
                schedulers=schedulers,
                reporter=reporter,
            ...
            with reporter.observe("valid") as sub_reporter:
                cls.validate_one_epoch(
                    model=dp_model,
                    iterator=valid_iter_factory.build_iter(trainer_options.max_epoch),
                    reporter=sub_reporter,
                    options=trainer_options,
                    distributed_option=distributed_option,
                )
            ...
            with autocast(
                ...
                **autocast_args,
                ...
                with reporter.measure_time("forward_time"):
                    teacher_retval = model.oracle_forward(**batch)
                    student_retval = model(**batch)

                    # Note(kamo):
                    # Supporting two patterns for the returned value from the model
                    #   a. dict type
                    if isinstance(teacher_retval, dict):
                        teacher_loss = teacher_retval["loss"]
                        teacher_stats = teacher_retval["stats"]
                        teacher_weight = teacher_retval["weight"]
                        teacher_optim_idx = teacher_retval.get("optim_idx")
                        if teacher_optim_idx is not None and not isinstance(teacher_optim_idx, int):
                            if not isinstance(teacher_optim_idx, torch.Tensor):
                                raise RuntimeError(
                                    "optim_idx must be int or 1dim torch.Tensor, "
                                    f"but got {type(teacher_optim_idx)}"
                                )
                            if teacher_optim_idx.dim() >= 2:
                                raise RuntimeError(
                                    "optim_idx must be int or 1dim torch.Tensor, "
                                    f"but got {teacher_optim_idx.dim()}dim tensor"
                                )
                            if teacher_optim_idx.dim() == 1:
                                for v in teacher_optim_idx:
                                    if v != teacher_optim_idx[0]:
                                        raise RuntimeError(
                                            "optim_idx must be 1dim tensor "
                                            "having same values for all entries"
                                        )
                                optim_idx = teacher_optim_idx[0].item()
                            else:
                                optim_idx = optim_idx.item()

                      # b. tuple or list type
                    else:
                        teacher_loss, teacher_stats, teacher_weight, teacher_pred = teacher_retval
                        student_loss, student_stats, student_weight, student_pred = student_retval
                        optim_idx = None

                    def logarithmic_interpolation(start, end, num_steps):
                        """
                        Logarithmic interpolation between start and end over num_steps steps.
                        """
                        assert num_steps > 1, "Number of steps must be greater than 1."
                        log_start = torch.log10(torch.tensor(start))
                        log_end = torch.log10(torch.tensor(end))
                        log_values = torch.linspace(log_start, log_end, num_steps)
                        return 10 ** log_values
                    values = logarithmic_interpolation(1e-8,1,250)

                    import torch.nn.functional as F
                    loss = teacher_loss +student_loss + values[reporter.get_epoch()-1] * F.mse_loss(teacher_pred,student_pred)

                stats, weight = student_stats, student_weight
