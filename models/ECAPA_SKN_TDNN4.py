...
class ResBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=8):
        ...
        #self.fwse   = fwSELayer(freq=40, reduction=reduction)
    ...
    def forward(self, x):
        residual = x
        #x   = x + self.penc(x) # adding positional encoding
        out = self.conv1(x)
        out = self.relu(out)
        out = self.bn1(out)
...
class ECAPA_CNN_TDNN(nn.Module):
    ...
    def forward(self, x, aug):
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                if aug == True:
                    x = self.specaug(x)
        x = x.unsqueeze(1)
        #print('LMFBEs       :  ', x.shape)
        
        x = self.frt_conv1(x)
        x = self.relu(x)
        x = self.frt_bn1(x)        
        #print('frt_conv1    :  ', x.shape)

        x = self.frt_block1(x)
        #print('frt_block1   :  ', x.shape)

        x = self.frt_block2(x)
        #print('frt_block2   :  ', x.shape)

        x = self.frt_conv2(x)
        x = self.relu(x)
        x = self.frt_bn2(x)
        #print('frt_conv2    :  ', x.shape)

        x = x.reshape((x.size()[0], -1, x.size()[-1]))
        #print('frt flatten  :  ', x.shape)
        #exit()
        ...
        x1 = self.layer1(x)
        #print('layer1 out   :  ', x1.shape)

        x2 = self.layer2(x+x1)
        #print('layer2 out   :  ', x2.shape)

        x3 = self.layer3(x+x1+x2)
        #print('layer3 out   :  ', x3.shape)

        x = self.layer4(torch.cat((x1,x2,x3),dim=1))
        x = self.relu(x)
        #print('layer4 out   :  ', x.shape)
...
def MainModel(eca_c=1024, eca_s=8, log_input=True, num_mels=80, num_out=192, **kwargs):
    model = ECAPA_CNN_TDNN(block=Bottle2neck, C=1024, model_scale=eca_s, log_input=log_input, num_mels=num_mels, num_out=num_out, resblock=ResBlock, **kwargs)
    return model
